...
const CHECKPOINTS = [0, 0.05, 0.1, 0.2, 0.25, 0.3, 0.4, 0.5, 0.6, 0.7, 0.75, 0.8, 0.9, 0.95, 1];
const FILM_POINTS = [0, 0.25, 0.5, 0.75, 1];
...
async function runLegDirection(page, { legIdx, from, to, wash, layout, engine, filmDir }) {
  ...
  for (const p of CHECKPOINTS) {
    if (p === 0) continue;
    const targetProgress = from + dir * p;
    const guessY = yFrom + (yTo - yFrom) * p;
    curY = await tickScrollTo(page, curY, guessY);
    for (let i = 0; i < 2; i++) {
      const cur = await page.evaluate(() => window.__landing.state().progress);
      const err = targetProgress - cur;
      if (Math.abs(err) < 0.006) break;
      const adj = err * (yTo - yFrom);
      curY = await tickScrollTo(page, curY, curY + adj, { tick: 60, gapMs: 8 });
    }
  ...
  return judge({ legIdx, from, to, dir, wash, layout, engine, refFrom, refTo, refFromNames, refToNames, samples });
}
...
function judge(ctx) {
  ...
  return { legIdx, from: SCENES[from], to: SCENES[to], fromIdx: from, toIdx: to, wash, layout, engine, clauses };
}
        ...
        for (const layout of LAYOUTS) {
          ...
          for (let legIdx = 0; legIdx < LEGS.length; legIdx++) {
            ...
            for (const [from, to] of [[a, b], [b, a]]) {
              const r = await runLegDirection(page, { legIdx, from, to, wash, layout: layout.name, engine: engineName, filmDir: FILM_DIR });
              results.push(r);
              printRow(r);
            }
          }
