#!/usr/bin/env node
// Verifies the <moss-stage>/<moss-scene> pages (site/ui/stage) in a real browser, in both
// Chromium and WebKit. Usage:
//   PLAYWRIGHT_MODULE=/path/to/node_modules/playwright/index.mjs node scripts/check-docs-stage.mjs <preview-url>
...
if (!input) {
  console.error('Usage: node scripts/check-docs-stage.mjs <preview-url>');
  process.exit(2);
}
...
// Every page with a stage worth exercising, and the markers it carries (in source order). Scene
// names are locale-independent (stage/README.md), so en and zh-hant share the same marker list
// for "Meet the editor" — its zh-hans twin is skipped here to keep the per-engine run short; the
// scenes it plays are identical to zh-hant's.
const STAGE_PAGES = [
  {
...
async function sceneData(name) {
  ...
  if (!cached) {
    cached = fetch(new URL(`ui/stage/scenes/${name}.json`, base)).then((res) => res.json());
    sceneCache.set(name, cached);
  }
  return cached;
}

// A generous upper bound on how long a scene can take to reach 'done' (stage/README.md, "Pace"):
...
const DWELL_MAX_MS = 600; // stage/README.md, "Pace": a right-click's dwell, the longer of the two
const PRESS_MS = 150;
async function paceBoundMs(name) {
  ...
  // is a fresh load — stage/README.md) — slower in a cold headless browser than the README's own
...
// Navigates and waits for the stage's iframe to exist — nothing more. The stage starts loading
// the default fixture as soon as it connects (see stage/README.md, "the stage is never empty"),
// but that load is asynchronous, so this helper still returns before it settles. Callers that
// need editor content ready wait for that themselves, via waitEditorReady.
async function gotoStage(browser, path, viewport) {
  const page = await browser.newPage({ viewport });
  await page.goto(new URL(path, base).href, { waitUntil: 'load' });
  await page.waitForSelector('moss-stage iframe');
  return page;
}

// Pairs with gotoStage for the common shape every stage check below follows: open a page at a
...
async function withPage(browser, path, viewport, fn) {
  const page = await gotoStage(browser, path, viewport);
  try {
    await fn(page);
  } finally {
    await page.close();
...
// For a page that carries no <moss-stage> at all (the divider-width check's control case).
...
async function waitEditorReady(page, timeout = 15000) {
  await page.waitForFunction(() => {
    const frame = document.querySelector('moss-stage iframe');
    return Boolean(frame?.contentWindow?.__editor) && frame.contentDocument?.documentElement.dataset.ready === '1';
  }, null, { timeout });
}

const editorText = (page) => page.evaluate(() => document.querySelector('moss-stage iframe').contentWindow.__editor.text);

// A marker's button holds an aria-hidden glyph (▶/■) plus a `.moss-scene__verb-text` span
// carrying the bare "Play"/"Stop" (moss-scene.js), visually hidden but readable off that class.
...
const markerState = (page, name) => page.evaluate((n) => {
  const marker = document.querySelector(`moss-scene[name="${n}"]`);
  const button = marker.querySelector('button');
  const verb = button.querySelector('.moss-scene__verb-text');
  return {
    button: (verb ?? button).textContent,
    status: marker.querySelector('.moss-scene__status').textContent,
  };
}, name);

async function clickMarker(page, name) {
  await page.evaluate((n) => {
    document.querySelector(`moss-scene[name="${n}"]`).scrollIntoView({ block: 'center' });
  }, name);
  await page.waitForTimeout(150);
  await page.evaluate((n) => {
    document.querySelector(`moss-scene[name="${n}"] button`).click();
  }, name);
}

async function pressKeyInEditor(page) {
  const frame = page.frameLocator('moss-stage iframe');
...
// playing (every marker reads Play), and the stage itself contains no button — the bar is gone;
// the text is both the list of scenes and the way to start them (stage/README.md).
async function checkRestState(browser, engine) {
  await withPage(browser, '/get-started/editor/', { width: 1440, height: 900 }, async (page) => {
    ...
    const counts = await page.evaluate(() => ({
      stages: document.querySelectorAll('moss-stage').length,
      iframes: document.querySelectorAll('moss-stage iframe').length,
      stageButtons: document.querySelectorAll('moss-stage button').length,
    }));
    assert(counts.stages === 1 && counts.iframes === 1, `[${engine}] expected exactly one moss-stage/iframe, got ${JSON.stringify(counts)}`);
    assert(counts.stageButtons === 0, `[${engine}] the stage itself carries a button; it must have no controls of its own: ${counts.stageButtons}`);
...
// alone (driver.js's PACE.travel*, stage/README.md "Pace") is already long enough to interrupt
...
async function checkInterruptStopsPlayback(browser, engine) {
  await withPage(browser, '/get-started/editor/', { width: 1440, height: 900 }, async (page) => {
    ...
    await page.waitForFunction(() => document.querySelector('moss-scene[name="tree"] .moss-scene__verb-text').textContent === 'Stop', null, { timeout: 5000 });
    await page.waitForTimeout(200);
    await pressKeyInEditor(page);

    await page.waitForFunction(() => document.querySelector('moss-scene[name="tree"] .moss-scene__verb-text').textContent === 'Play', null, { timeout: 5000 });
...
// The framed editor's own UI strings follow the page's locale (harvest.js's `lang` passthrough,
// stage/README.md item 2): the context menu's "Versions…" row reads in English on the English
...
async function readVersionsMenuLabel(browser, path) {
  ...
  await withPage(browser, path, { width: 1440, height: 900 }, async (page) => {
    ...
    label = await page.evaluate(() => {
      const doc = document.querySelector('moss-stage iframe').contentDocument;
...
async function checkFixturesFrameDifferentProjects(browser, engine) {
  const leafPath = async (path) => {
    ...
    await withPage(browser, path, { width: 1440, height: 900 }, async (page) => {
      await waitEditorReady(page);
      text = await page.evaluate(() => document.querySelector('moss-stage iframe').contentDocument.querySelector('.seg.leaf.bc')?.dataset.path ?? '');
    });
    return text;
  };
...
async function checkVersionsDiffShowsOwnText(browser, engine) {
  await withPage(browser, '/get-started/editor/', { width: 1440, height: 900 }, async (page) => {
    ...
    const diffText = await page.evaluate(() => document.querySelector('moss-stage iframe').contentDocument.querySelector('.moss-versions__source').textContent);
    ...
    await page.waitForFunction(() => document.querySelector('moss-scene[name="versions"] .moss-scene__verb-text').textContent === 'Play', null, { timeout: await paceBoundMs('versions') });
  });
}
...
// load at a time"). Every Play now navigates afresh regardless of fixture (stage/README.md,
// "Play"), and app-editor.html reads no fixture-selecting query params at all (harvest.js), so the
...
async function checkSecondLoadDoesNotNavigate(browser, engine) {
  ...
  try {
    ...
    await page.route('**/ui/stage/scenes/properties.json', async (route) => {
      await aHeld;
      await route.fulfill({
    ...
    await page.waitForFunction(() => {
      const stage = document.querySelector('moss-stage');
      return stage.phase === 'loading' && stage.load?.name === 'properties';
    }, null, { timeout: 5000 });
    ...
    // ever resolves.
    await clickMarker(page, 'tree');
    await page.waitForFunction(() => document.querySelector('moss-stage').phase === 'done', null, { timeout: await paceBoundMs('tree') });
    await page.evaluate(() => { document.querySelector('moss-stage iframe').contentWindow.__mossNoRenavigateProbe = true; });

    releaseA();
    // Let the released, superseded fetch and whatever it does to the stage's state actually
    ...
    const stillSameWindow = await page.evaluate(() => document.querySelector('moss-stage iframe').contentWindow.__mossNoRenavigateProbe === true);
    assert(stillSameWindow, `[${engine}] the superseded "properties" load navigated the shared iframe again once released`);

    const load = await page.evaluate(() => {
      const stage = document.querySelector('moss-stage');
      const step = stage.load?.scene?.steps?.[0];
      return { name: stage.load?.name, phase: stage.phase, step: step?.click ?? step?.context };
    });
...
async function checkLoadFailureRetries(browser, engine) {
  await withPage(browser, '/get-started/editor/', { width: 1440, height: 900 }, async (page) => {
    await waitEditorReady(page);
    let requestCount = 0;
    await page.route('**/ui/stage/scenes/properties.json', (route) => {
      requestCount += 1;
    ...
    await clickMarker(page, 'properties');
    await page.waitForFunction(() => document.querySelector('moss-scene[name="properties"] .moss-scene__status')?.textContent?.length > 0, null, { timeout: 15000 });
    ...
    await page.evaluate(() => document.querySelector('moss-scene[name="properties"] button').click());
    await page.waitForFunction(() => document.querySelector('moss-scene[name="properties"] .moss-scene__status')?.textContent === '', null, { timeout: 15000 });
    ...
    await page.waitForFunction(() => document.querySelector('moss-scene[name="properties"] .moss-scene__verb-text').textContent === 'Play', null, { timeout: await paceBoundMs('properties') });
...
// pinned fits a 1440×800 viewport; at 390px no horizontal overflow and the stage follows the
// first paragraph.
...
async function checkStickyLayout(browser, engine) {
  await withPage(browser, '/get-started/editor/', { width: 1440, height: 900 }, async (page) => {
    ...
    for (const frac of [0, 0.1, 0.25, 0.4, 0.55, 0.7]) {
      ...
      const rect = await page.evaluate(() => document.querySelector('moss-stage').getBoundingClientRect());
      assert(rect.right <= 1440 + 1, `[${engine}] stage overflowed the viewport at ${frac}: ${JSON.stringify(rect)}`);
      assert(rect.top >= -1 && rect.bottom <= 900 + 1, `[${engine}] stage left the viewport vertically at ${frac} (sticky not holding): ${JSON.stringify(rect)}`);
      tops.push(rect.top);
      const overlap = await page.evaluate((stageRect) => [...document.querySelectorAll('article > :not(moss-stage)')].some((el) => {
        const r = el.getBoundingClientRect();
        return r.left < stageRect.right && r.right > stageRect.left && r.top < stageRect.bottom && r.bottom > stageRect.top;
      }), rect);
      assert(!overlap, `[${engine}] prose overlaps the stage at scroll fraction ${frac}`);
    }
    ...
    assert(spread < 1, `[${engine}] stage top did not stay clamped while scrolling (not actually sticky): ${JSON.stringify(stuckTops)}`);
  });

  // A shorter viewport (1440×800): the pinned stage must still fit, top offset present and bottom
  // not cut off, not just the taller 900px case above. Scrolls by a fraction of the page's own
  // scrollable range, the same way the 900px case above does, rather than a hard-coded pixel
  // offset — a fixed 2000px landed past the article's sticky range once the page got shorter (the
  // stage's top read -46 in WebKit), so this recomputes the fraction against whatever height the
  ...
  await withPage(browser, '/get-started/editor/', { width: 1440, height: 800 }, async (page) => {
    ...
    await page.waitForTimeout(150);
    const rect = await page.evaluate(() => document.querySelector('moss-stage').getBoundingClientRect());
    assert(rect.top > 0, `[${engine}] pinned stage has no top offset at 1440×800: ${JSON.stringify(rect)}`);
    assert(rect.bottom <= 800 + 1, `[${engine}] pinned stage's bottom is cut off at 1440×800: ${JSON.stringify(rect)}`);
  });
}
...
async function checkThemeFollowsPage(browser, engine) {
  await withPage(browser, '/get-started/editor/', { width: 1440, height: 900 }, async (page) => {
    ...
    await page.evaluate(() => { if (document.documentElement.dataset.theme !== 'light') window.toggleTheme(); });
    await page.waitForFunction(() => document.documentElement.dataset.theme === 'light');
    await page.waitForFunction(() => document.querySelector('moss-stage iframe').contentDocument.documentElement.dataset.theme === 'light');
    const lightBg = await page.evaluate(() => getComputedStyle(document.querySelector('moss-stage iframe').contentDocument.body).backgroundColor);

    await page.evaluate(() => window.toggleTheme());
    await page.waitForFunction(() => document.documentElement.dataset.theme === 'dark');
    await page.waitForFunction(() => document.querySelector('moss-stage iframe').contentDocument.documentElement.dataset.theme === 'dark');
    ...
    await page.evaluate(() => window.toggleTheme());
    await page.waitForFunction(() => document.querySelector('moss-stage iframe').contentDocument.documentElement.dataset.theme === 'light');
    const backToLightBg = await page.evaluate(() => getComputedStyle(document.querySelector('moss-stage iframe').contentDocument.body).backgroundColor);
...
// clipped by the stage's box.
async function checkToolbarShowsOnFocusAndFits(browser, engine) {
  await withPage(browser, '/get-started/editor/', { width: 1440, height: 900 }, async (page) => {
    await waitEditorReady(page);
    const frame = page.frameLocator('moss-stage iframe');
    const beforeFocus = await page.evaluate(() => document.querySelector('moss-stage iframe').contentDocument.querySelector('.insert-bar')?.classList.contains('visible'));
    assert(!beforeFocus, `[${engine}] toolbar is already visible before the editor has focus — the trigger is no longer focus`);

    await frame.locator('.cm-content').click();
    await page.waitForFunction(() => document.querySelector('moss-stage iframe').contentDocument.querySelector('.insert-bar.visible'), null, { timeout: 5000 });

    const iframeRect = await page.evaluate(() => document.querySelector('moss-stage iframe').getBoundingClientRect());
    const barRect = await page.evaluate(() => {
      const r = document.querySelector('moss-stage iframe').contentDocument.querySelector('.insert-bar').getBoundingClientRect();
      return { top: r.top, bottom: r.bottom, left: r.left, right: r.right };
    });
    assert(
      barRect.top >= 0 && barRect.bottom <= iframeRect.height + 1 && barRect.left >= 0 && barRect.right <= iframeRect.width + 1,
      `[${engine}] toolbar is clipped by the stage's box: iframe=${JSON.stringify(iframeRect)} bar(iframe-local)=${JSON.stringify(barRect)}`,
    );
  });
}
...
async function checkNarrow(browser, engine) {
  await withPage(browser, '/get-started/editor/', { width: 390, height: 844 }, async (page) => {
    ...
    assert(result.position !== 'sticky', `[${engine}] stage is sticky at 390px, expected inline: ${result.position}`);
    assert(result.pTop !== null && result.pTop < result.stageTop, `[${engine}] the opening paragraph does not precede the stage at 390px: ${JSON.stringify(result)}`);
    assert(result.pText.length > 0 && result.pBottom > 0 && result.pTop < result.viewportHeight,
      `[${engine}] the first viewport at 390px shows no prose before the stage: ${JSON.stringify(result)}`);
  });
}

// h. At 1440px, the header divider's, the footer divider's, and the series-nav divider's
// left/right edges match the article's outer edges within 1px, on "Meet the editor" (a stage
// page) and on a docs page with no stage alike. moss's `.container` reads --moss-site-max-width,
...
async function checkDividerWidthsMatchArticle(browser, engine) {
  const widths = (page) => page.evaluate(() => {
    ...
    const expectedWideWidth = Math.min(
      pxOf('calc(var(--moss-content-width) + var(--moss-space-lg) + var(--moss-stage-width))'),
  ...
  await withPage(browser, '/get-started/editor/', { width: 1440, height: 900 }, async (page) => {
    ...
    assertMatch(w, 'on a page with a stage');
    assert(Math.abs(w.article.width - w.expectedWideWidth) <= 2,
      `[${engine}] a page with a stage is not the wide formula's width (moss's own --moss-site-max-width default would ALSO be wider than a plain page, so this checks the exact value, not just "wider"): got=${w.article.width} expected=${w.expectedWideWidth}`);
  });
  await withPlainPage(browser, '/docs/writing/structure/', { width: 1440, height: 900 }, async (page) => {
    assertMatch(await widths(page), 'on a page with no stage');
  });
}

// j. "Pressing Play moves nothing" (stage/README.md) on the wide layout: scrollY, the opening
// paragraph, the stage, and the pressed button's own rect must not move by more than 1px at any
...
function measureStillness(page, name, durationMs) {
  return page.evaluate(({ n, duration }) => new Promise((resolve) => {
    function snap() {
      ...
      return { scrollY: window.scrollY, firstP: pos(firstP), stage: pos(stage), button: pos(button) };
    }
    const base = snap();
    const samples = [base];
    ...
    const interval = setInterval(() => {
      ...
      if (performance.now() - start > duration) {
        ...
        resolve({
          ...
          maxStageDelta: maxDelta((s) => s.stage),
...
async function checkPlayDoesNotShiftPage(browser, engine) {
  ...
  await withPage(browser, '/get-started/editor/', { width: 1440, height: 900 }, async (page) => {
    ...
    assert(
      tree.maxScrollDelta <= 1 && tree.maxFirstPDelta <= 1 && tree.maxStageDelta <= 1 && tree.maxButtonDelta <= 1,
      `[${engine}] Play shifted the page for marker "tree": ${JSON.stringify(tree)}`,
    );
  });

  // Marker "versions", scrolled down into view first (so the stage is pinned/sticky — the exact
  ...
  await withPage(browser, '/get-started/editor/', { width: 1440, height: 900 }, async (page) => {
    await waitEditorReady(page);
    await page.evaluate(() => document.querySelector('moss-scene[name="versions"]').scrollIntoView({ block: 'center' }));
    await page.waitForTimeout(150);
    const v = await measureStillness(page, 'versions', await paceBoundMs('versions'));
    assert(
      v.maxScrollDelta <= 1 && v.maxFirstPDelta <= 1 && v.maxStageDelta <= 1 && v.maxButtonDelta <= 1,
      `[${engine}] Play shifted the page for marker "versions": ${JSON.stringify(v)}`,
    );
  });
}

// The consolidated playback check: every marker on every page in STAGE_PAGES plays to 'done' with
...
// what a scene's steps specifically leave behind — stage/README.md's own module boundary already
// keeps a scene's *effect* out of player.js/moss-stage.js's concern, so this checks only what the
// stage contract promises for every scene: it plays, and it settles back to Play.
async function checkEveryMarkerPlaysToDone(browser, engine) {
  for (const { path, markers } of STAGE_PAGES) {
    await withPage(browser, path, { width: 1440, height: 900 }, async (page) => {
      const pageErrors = [];
      ...
      for (const name of markers) {
        ...
        // "播放"/"停止", not "Play"/"Stop" — so this reads moss-stage's own `phase` property
        ...
        await page.waitForFunction((n) => {
          const stage = document.querySelector('moss-stage');
          return stage.load?.name === n && stage.phase === 'playing';
        }, name, { timeout: 5000 });
        const bound = await paceBoundMs(name);
        await page.waitForFunction((n) => {
          const stage = document.querySelector('moss-stage');
          return stage.load?.name === n && stage.phase !== 'playing' && stage.phase !== 'loading';
        }, name, { timeout: bound });
        const phase = await page.evaluate(() => document.querySelector('moss-stage').phase);
        const status = await page.evaluate((n) => document.querySelector(`moss-scene[name="${n}"] .moss-scene__status`)?.textContent ?? '', name);
