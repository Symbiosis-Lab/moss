...
- A long video's HLS delivery ladder no longer produces a single rung file too big for moss hosting to accept, which used to fail the whole publish. Each rung (and each audio track) is uploaded as one file, so its size is that rung's bitrate times the video's whole duration — a rung that fit the source's width just fine could still be hundreds of megabytes for a long clip. The ladder now also drops a rung whose file would be too big, the same way it already drops a rung too wide for the source, and the build log names the video when that happens so a missing top-quality rung isn't a mystery.
