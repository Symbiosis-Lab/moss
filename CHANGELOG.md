...
- Renaming or moving a folder that has no page of its own — no `index.md`, no self-named note, just moss's automatically generated folder page — now updates a bare `[[Folder]]` link to it along with everything else; before, that link was invisible to rename and was left pointing at the folder's old name.
- Renaming a folder that has its own same-named page (`Trip/Trip.md`) no longer breaks links to that page written as its bare filename (`[text](Trip.md)`, `[[Trip]]`, or a gallery/cover path) — they're renamed along with everything else instead of being left pointing at a file that no longer exists. Renaming or moving a file also now updates that file's own links to other pages when the move changes what a relative link like `../notes.md` points at, and a link is only ever rewritten when it actually needs to be, so files with nothing to change are left untouched.
- A build-time notice (a symlink moss had to skip, or a `config.toml` schema newer than this build understands) now clears itself the moment the underlying problem is fixed and the site rebuilds, instead of staying shown until the folder is closed and reopened. The build now says explicitly when it checked and found nothing, rather than staying silent — silence used to be indistinguishable from "never checked". A clean `moss build` on the CLI no longer prints a spurious extra status line for these checks; they only speak when they have something to report.
- A per-file notice about an image or video (for example, one shipped unoptimized because a size or format target wasn't met) now clears once the image or video is fixed, even on a rebuild where most other images and videos were unchanged and skipped. Before, fixing one file while its siblings sat untouched could leave that file's notice showing until the folder was closed and reopened, because the build only reported on the files it actually re-processed that round. Deleting the file it was about clears the notice too.
...
- Paragraph, heading and figure spacing in articles now scales with the reader's text size and the CJK line-height bump, instead of staying a fixed number of pixels while the text itself grows. Before, the gap between two paragraphs stayed the same 24px whether body text was 16px or 24px, so at the largest reader font size a paragraph break read as barely more than a line's own leading; the gap above a `##` heading also matched the gap below a photo exactly, so a section starting right after an image didn't read as a break at all. At the default text size the change is invisible — spacing lands within a pixel of before — except around a photo, which is now about 9px tighter.
...
- On vertically typeset pages, an image or embed with a width (`|body`, `|wide`, `|page`, `|screen`) no longer covers the text that follows it; it stands in the column like any other figure, and `wide`/`page`/`screen` no longer leave a wide blank gap beside it. A `|wide|align-right` image or listing is centred on the column again instead of being pushed off the left edge of the page, and on right-to-left pages a `|wide` band is centred instead of sitting off to one side. An image or embed written with both a width token and a percent (`|wide|40%`) now takes the percent; embeds used to land partly outside the text column, and images used to ignore the percent.
- On vertically typeset sites, a standalone image in the body now fills the column's height. Before, a large image stopped at 756px wide, so a landscape painting sat short of the column and a handscroll shrank to a thin strip.
- On vertically typeset pages, body images now load a copy sharp enough for the size they are drawn at. They used to pick the copy meant for a horizontal column, so a painting drawn about 800px wide loaded the 800px copy and looked soft on a high-density screen, and a wide scroll loaded far less than it needed.
- Long, narrow images such as handscrolls and hanging scrolls now keep their detail when published. Images were capped at 2400px on the long side, so a handscroll became a strip about 100px tall. Now the short side of an image longer than 2:1 is kept at 1200px or more, up to WebP's 16383px limit; images up to 2:1 publish exactly as before. The first build after upgrading re-encodes every image once, and the files come out the same for images up to 2:1. The fallback copy for browsers without WebP, and link-preview images, still use the old cap.
...
- `sitemap.xml` no longer lists pages whose files were deleted outside moss, for example by a `git pull`. The first build after such a deletion stopped publishing those pages but kept them in the sitemap, so search engines were sent to addresses that no longer exist. A site's first build now lists the same pages as every later build; the home page, the photo, video and experiment collection pages, notebook pages and `.html` pages from the folder used to be missing from it. Redirect pages and the email-subscription pages are no longer listed.
- A series whose pages share a date now reads, through its previous/next links, in the same order its folder's page lists them, and that order is the same on every build. Before, the links could run 4, 1, 2, 3 on one build and 4, 3, 2, 1 on the next. Pages given the same `weight:`, or the same title in a title-sorted folder, also keep one order now.
- A long or low-bitrate video's HLS delivery ladder no longer produces a single rung file too big for moss hosting to accept, and no longer re-encodes a rung above the picture quality the source itself actually has — the first used to fail the whole publish, and the second just wasted upload budget on a bigger file that looked no sharper. Each rung (and each audio track) is uploaded as one file, so its size is that rung's bitrate times the video's whole duration — a rung that fit the source's width just fine could still be hundreds of megabytes for a long clip. The ladder now drops a rung whose file would be too big, the same way it already drops a rung too wide for the source, and separately caps a rung's bitrate at what the source's own video stream carries, dropping the rung instead when that cap would leave it barely distinguishable from the rung below; the build log names the video when either narrowing is why its top quality is missing. Changing the per-file budget, or upgrading moss to a release that adjusts either of the two narrowings above, no longer re-encodes every streaming video on a site: only a video whose own streaming ladder would actually come out different is re-encoded, so a 13 Mbps 1080p source that neither narrowing ever touched is left alone. Ladders from before this change are picked up automatically, at no extra cost to a build that has nothing else to do: a video whose ladder already matches what today's settings would produce is adopted as-is on its next build, and only a video whose ladder needs to change re-encodes, once.
- An unchanged video's HLS delivery ladder that went missing from a rebuild — while its progressive copy and poster were untouched — is now relinked from the build cache instead of silently disappearing from the published site; before, only the progressive copy and poster recovered this way, and the ladder stayed gone, with a build log warning every build afterward, until the video was edited to force a full re-encode. A video re-encoded because its own file changed also keeps its previous ladder live in the meantime, instead of it dropping out while the new one is produced. A video whose ladder has fewer quality levels than the maximum no longer causes those spurious build log warnings either.
- A video's HLS delivery ladder that shrank from an earlier build — a rung dropped by a tighter per-file budget, a table change, or the cap on a rung's bitrate described above — no longer publishes the dropped rung's leftover files alongside the smaller ladder; before, those files were still uploaded on publish, and one of them being oversized could fail the whole publish even though the ladder moss just produced was fine on its own. A site whose build output already has this problem stops publishing the extra files starting with its very next build, no re-encode needed, and the build's routine cleanup reclaims their disk space over the builds that follow.
- A page that claims a term kind (`author_page:`, `tag_page:`, and friends) now shows its own cover beside its title, the same layout a folder-index page claiming the same term already used; before, the cover only ever reached the page's share/OG metadata once the claim added that term's listing below it.
...
- `FileChangeEvent.modified_paths` is now filled in: the page sources (markdown, slot files included) whose bytes changed in the rebuild, plus `.moss/config.toml`, `.moss/places.toml` and the theme `style.css`/`script.js` — the only other sources read synchronously enough for the diff to be race-free — deduped against `source_creates` and `source_renames`. It was documented as logging-only and was always `None`; a rebuild whose only change is such an edit now emits an event too.
