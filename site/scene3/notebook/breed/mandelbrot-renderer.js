(() => {
  ...
  const MAX=4096, RATE=2.2, CYCLE=24, OVERLAP=6, BASE=4; // bounded depth windows overlap before the fixed-point perturbation loses precision
  ...
  uniform vec2 size;uniform float logZoom,nextZoom,blend;uniform sampler2D orbit;
  vec3 palette(float x){vec3 a=vec3(.48,.39,.54),b=vec3(.48,.43,.40),c=vec3(1.,.74,.58),d=vec3(.02,.13,.28);return a+b*cos(6.283185*(c*x+d));}
  vec3 renderDepth(vec2 p,float depth){
    float scale=exp(-depth);vec2 dc=p*3.1*scale,dz=vec2(0),z=vec2(0);float n=0.,trap=8.;
    ...
    float escaped=step(n,4095.);float smoothN=n-log2(log2(max(dot(z,z),1.0001)));float band=fract(smoothN*.037+depth*.017);
    ...
    vec3 col=mix(inside,exterior,escaped);return pow(max(col+.12*palette(.12)*exp(-18.*trap),0.),vec3(.82));
  }
  void main(){vec2 p=(uv-.5)*vec2(size.x/size.y,1.);vec3 a=renderDepth(p,logZoom),b=blend>0.001?renderDepth(p,nextZoom):a;outColor=vec4(mix(a,b,blend),1.);}`;
  ...
  if(gl){const shader=(type,src)=>{const s=gl.createShader(type);gl.shaderSource(s,src);gl.compileShader(s);if(!gl.getShaderParameter(s,gl.COMPILE_STATUS))throw Error(gl.getShaderInfoLog(s));return s};program=gl.createProgram();gl.attachShader(program,shader(gl.VERTEX_SHADER,vertex));gl.attachShader(program,shader(gl.FRAGMENT_SHADER,fragment));gl.linkProgram(program);if(!gl.getProgramParameter(program,gl.LINK_STATUS))throw Error(gl.getProgramInfoLog(program));const buf=gl.createBuffer();gl.bindBuffer(gl.ARRAY_BUFFER,buf);gl.bufferData(gl.ARRAY_BUFFER,new Float32Array([-1,-1,1,-1,-1,1,1,1]),gl.STATIC_DRAW);const pos=gl.getAttribLocation(program,'p');gl.enableVertexAttribArray(pos);gl.vertexAttribPointer(pos,2,gl.FLOAT,false,0,0);const tex=gl.createTexture();gl.bindTexture(gl.TEXTURE_2D,tex);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.NEAREST);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.NEAREST);gl.texImage2D(gl.TEXTURE_2D,0,gl.RG32F,MAX,1,0,gl.RG,gl.FLOAT,ref);loc={size:gl.getUniformLocation(program,'size'),zoom:gl.getUniformLocation(program,'logZoom'),next:gl.getUniformLocation(program,'nextZoom'),blend:gl.getUniformLocation(program,'blend'),orbit:gl.getUniformLocation(program,'orbit')}}
  const active=()=>parentVisible&&intersecting&&!document.hidden;
  function fallbackDraw(){const w=180,h=82;if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h}const im=fallback.createImageData(w,h),blend=Math.max(0,Math.min(1,(logZoom-(CYCLE-OVERLAP))/OVERLAP)),depthA=BASE+logZoom,depthB=depthA-CYCLE,scaleA=3.1*Math.exp(-depthA),scaleB=3.1*Math.exp(-depthB);for(let y=0;y<h;y++)for(let x=0;x<w;x++){const u=x/w-.5,v=y/h-.5;const sample=(scale)=>{const cr=-.7436438870371587+u*scale*w/h,ci=.131825904205312+v*scale;let zr=0,zi=0,n=0;for(;n<384&&zr*zr+zi*zi<256;n++){const q=zr*zr-zi*zi+cr;zi=2*zr*zi+ci;zr=q}return n/384};const t=sample(scaleA)*(1-blend)+sample(scaleB)*blend,o=(y*w+x)*4;im.data[o]=45+Math.floor(205*t);im.data[o+1]=18+Math.floor(105*(1-t));im.data[o+2]=70+Math.floor(165*t);im.data[o+3]=255}fallback.putImageData(im,0,0)}
  function draw(now){if(!gl||!program){if(fallback){fallbackDraw();depth.textContent='log zoom '+(BASE+logZoom).toFixed(1);status.textContent=reduce?'paused · reduced motion':'running · canvas fallback';lastDraw=now}return;}const mobile=top.innerWidth<=900,dpr=mobile?1:Math.min(1.35,devicePixelRatio||1),rect=canvas.getBoundingClientRect(),w=Math.max(1,Math.round(Math.min(rect.width,mobile?240:rect.width)*dpr)),h=Math.max(1,Math.round(rect.height*dpr));if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;gl.viewport(0,0,w,h)}gl.useProgram(program);gl.uniform2f(loc.size,w,h);gl.uniform1f(loc.zoom,BASE+logZoom);gl.uniform1f(loc.next,BASE+logZoom-CYCLE);gl.uniform1f(loc.blend,Math.max(0,Math.min(1,(logZoom-(CYCLE-OVERLAP))/OVERLAP)));gl.uniform1i(loc.orbit,0);gl.drawArrays(gl.TRIANGLE_STRIP,0,4);depth.textContent='log zoom '+(BASE+logZoom).toFixed(1);status.textContent=reduce?'paused · reduced motion':'running';lastDraw=now}
  ...
  function frame(now){raf=0;if(!active())return stop();const dt=Math.max(0,Math.min(.1,(now-last)/1000));last=now;logZoom=(logZoom+dt*RATE)%CYCLE;const interval=top.innerWidth<=900?66:32;if(now-lastDraw>=interval)draw(now);schedule()}
  ...
  window.__notebook={pause:()=>{parentVisible=false;visibility()},resume:()=>{parentVisible=true;visibility()},setLogZoom:v=>{logZoom=((Number(v)||0)-BASE+CYCLE)%CYCLE;draw(performance.now())},state:()=>({visible:active(),logZoom:BASE+logZoom,reduced:reduce,referenceOrbit:MAX,precision:'Q160 fixed-point reference + float perturbation with 6-unit overlap',testedHorizon:BASE+CYCLE,initialRate:RATE,trajectory:'overlapping bounded depth loop'})};
})();
