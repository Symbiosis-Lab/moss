// Everything moss-stage.js and driver.js need to know about the harvested editor document
// (site/ui/app-editor.html): its named click/context targets, its theme attributes, and how to
// frame it. Moves into moss-desktop/scripts/landing/ once the driver gains a `load(fixture)`
// method (see stage/README.md, "The harvested document") — it belongs with the harvest, not the
// reader-facing stage.
...
// (moss-desktop/scripts/landing/app-editor-boot-params.ts) from the `?lang=` query string alone —
...
// so moss-stage.js's `scene.fixture ?? DEFAULT_FIXTURE` shape survives unchanged; frameUrl ignores
...
/** Resolves a named target against the harvested document. Throws if the name or the element is missing. */
export function resolveTarget(doc, name) {
  const el = findTarget(doc, name);
  if (!el) throw new Error(`Click target "${name}" was not found in the editor document`);
  return el;
}
...
// them — which is why moss-stage.js hands this one theme string rather than two.
export function setTheme(doc, theme) {
  doc.documentElement.dataset.theme = theme;
  doc.documentElement.dataset.chromeTheme = theme;
}

/** The URL that opens the harvested document. `fixture` is accepted only to keep the stage's one
 ...
 * editor.html's `root`/`file`/`date` params, and the one line of moss-stage.js that varied this
 ...
 * (traditional characters for `zh-hant`, simplified for `zh-hans`) otherwise. moss-stage.js is the
...
/** Resolves once `win.__editor` exists and the harvested document marks itself ready. */
export function waitForEditorReady(win, doc) {
