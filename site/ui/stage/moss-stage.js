// The one real moss editor a documentation page shows. Owns the iframe, its driver, and the
// pointer overlay. Loads the default fixture as soon as it connects, and plays a scene when a
// marker asks it to, by reacting to `moss-scene-play` / `moss-scene-stop` events bubbled to
// `document` (see moss-scene.js) and reporting back with `moss-stage-state`. Knows nothing about
// where markers are, how the page is laid out, query parameters, selectors, or theme attribute
// names — see stage/README.md's module table.
import { playChain } from './player.js';
import { createDriver } from './driver.js';
import { DEFAULT_FIXTURE, setTheme, frameUrl, waitForEditorReady } from './harvest.js';
import { string } from './strings.js';

const MODULE_BASE = new URL('.', import.meta.url);

if (!document.querySelector('link[data-moss-stage]')) {
  const link = document.createElement('link');
  link.rel = 'stylesheet';
  link.href = new URL('moss-stage.css', MODULE_BASE).href;
  link.dataset.mossStage = '';
...
/**
 * Walks `scene.after` (stage/README.md, "Scene format"), loading each ancestor by name, and
 * returns the chain ordered eldest ancestor first, `name`/`scene` — the one actually requested —
 * last: the order player.js's `playChain` runs in. This is the "scene loading" half of `after`
 * composition (the module table in stage/README.md); running the resolved chain is player.js's
 * job. Throws loudly if a name repeats: an `after` cycle would otherwise fetch forever.
 */
...
/** The page's own effective theme (stage/README.md, "Fitting the page"). This site's toggle
...
/** Reports a named scene's phase to whichever <moss-scene> is listening (stage/README.md,
 * "Who controls the stage"). The stage never looks for markers; this is the only channel back. */
function announce(name, phase) {
  document.dispatchEvent(new CustomEvent('moss-stage-state', { detail: { name, phase } }));
}

class MossStage extends HTMLElement {
  connectedCallback() {
    ...
    this.innerHTML = `
      <div class="moss-stage__frame-wrap">
        <iframe class="moss-stage__frame" title="${string(this.locale, 'frame')}" loading="lazy"></iframe>
        <div class="moss-stage__pointer" hidden aria-hidden="true">
          <span class="moss-stage__pointer-ring"></span>
          <span class="moss-stage__pointer-glyph"></span>
          <span class="moss-stage__pointer-dot"></span>
        </div>
      </div>`;

    this.frame = this.querySelector('.moss-stage__frame');
    this.frameWrap = this.querySelector('.moss-stage__frame-wrap');
    this.pointerEl = this.querySelector('.moss-stage__pointer');
    ...
    document.addEventListener('moss-scene-play', this.onScenePlayEvent, { signal });
    document.addEventListener('moss-scene-stop', this.onSceneStopEvent, { signal });
    ...
    // Theme follows the page (stage/README.md, "Fitting the page"): re-applied to every freshly
    // loaded iframe document in loadFrame, and here for a page-driven change (the reader's own
    // toggle) to an already-loaded editor.
    this.themeObserver = new MutationObserver(() => this.syncTheme());
    this.themeObserver.observe(document.documentElement, { attributes: true, attributeFilter: ['data-theme'] });

    // "The stage is never empty" (stage/README.md): loaded on connect, no marker involved. A
    ...
    this.loadFrame(DEFAULT_FIXTURE, this.defaultLoad.signal)
      .catch((error) => {
        if (!this.defaultLoad.signal.aborted) console.error('moss-stage: could not load the default fixture', error);
      });
  }
  ...
  /**
   * One load at a time (stage/README.md): starting any scene here abandons whichever load was
   * previously in flight or playing — including the connect-time default-fixture load — by
  ...
  async startScene(name) {
    ...
    try {
      ...
      await this.loadFrame(scene.fixture ?? DEFAULT_FIXTURE, controller.signal);
    } catch (error) {
      if (controller.signal.aborted) return;
      console.error('moss-stage: could not load scene', name, error);
      this.phase = 'error';
      announce(name, 'error');
      return;
    }
    ...
    // `open` is optional (stage/README.md, "Scene format"): a scene that never names it plays
    // against whatever the fresh fixture load itself already contains — its own authored piece —
    // rather than being silently wiped to empty first. Discovered building the "versions" scene:
    // every scene up to then set `"open": ""` out of habit, which happened to be harmless because
    // none of them cared about the document's actual text, but the versions scene's own "Save a
    // version" step snapshots whatever text is live at that moment — wiping it first meant it
    // could only ever save and diff an empty document, no test having read that far until
    // check-docs-stage.mjs's checkVersionsDiffShowsOwnText did.
    if (scene.open !== undefined) this.driver.setText(scene.open);
    this.settleView();
    // "focus goes into the editor with preventScroll" (stage/README.md, "Pressing Play moves
    // nothing") — moves keyboard/AT focus into the live demo on every Play, without `focus()`'s
    // own default of scrolling the target into view, which would be a second source of motion on
    // top of settleView above.
    this.frame.focus({ preventScroll: true });
    await this.runPlayback(controller.signal, name);
  }

  /** "Pressing Play moves nothing" (stage/README.md): on the wide layout the stage sits in a
   ...
   * current stuck one, so once the reader has scrolled far enough for the stage to actually be
   * pinned, `{ block: 'nearest' }` "corrects" a scroll that was never wrong — confirmed by
   * measuring a ~60px scrollY jump on Play with the stage fully inside the viewport throughout.
   ...
   * narrow (non-sticky) layout may scroll at all, and only when the stage is mostly out of the
  ...
  /** Every Play is a fresh load (stage/README.md, "Who controls the stage"): a scene can leave the
   * app anywhere — versions mode, an open menu, a different file — and reloading the iframe is the
   * only reset that is true for all of them, so this always navigates rather than checking whether
   * anything would actually differ. Checks `signal` before touching `iframe.src` at all, not only
   * after the ready-wait: a load this call's own caller has already superseded must not navigate
   * the shared iframe out from under whichever load won, however late its own fetch resolves. */
  async loadFrame(fixture, signal) {
    ...
    const ready = new Promise((resolve, reject) => {
      const onLoad = () => {
        ...
        waitForEditorReady(this.frame.contentWindow, this.frame.contentDocument).then(resolve, reject);
      };
      this.frame.addEventListener('load', onLoad);
    });
    this.frame.src = frameUrl(fixture, this.locale);
  ...
  syncTheme() {
    ...
    setTheme(this.frame.contentDocument, pageTheme());
  }
  ...
  /** "The reader outranks the script" (stage/README.md): any pointer or key press inside the
   * editor stops a playing scene at once and keeps what is there — Play always restarts from the
   * opening state rather than resuming. Idle browsing (no scene playing) is not interrupted.
   *
   ...
   * they never ran — the stage still reported 'done' with no error, since this same handler is
   ...
   * `HTMLElement.prototype.focus` patched to force `preventScroll` (moss-desktop,
   * scripts/landing/app-editor-boot-params.ts) — neither a 'focus' listener nor a patched
   * `scrollIntoView` on the iframe's own window ever fires for it, so whatever WebKit does here
   * bypasses both of the DOM APIs a script can intercept. Comparing `window.scrollY` against the position captured
   * once at the scene's own start is the one thing left that reaches a cause this module cannot
   * name, run from two places for two different failure windows: this 'scroll' listener corrects
   * synchronously, before the next paint, closing most of the gap; `guardScrollFrame` below is the
   * once-a-frame fallback for a drift that arrives some other way a 'scroll' event doesn't cover.
   * Neither can outlive its own run or fight a reader's own scroll once a scene has finished: this
   * one checks `phase === 'playing'` via `scrollGuardY`, only set for that duration;
   * `guardScrollFrame` below additionally carries the load's own signal, so removing the element
   * mid-scene (disconnectedCallback aborting that signal) stops its `requestAnimationFrame` loop
   * even if `phase` itself never gets the chance to change. */
  onWindowScroll = () => {
  ...
  async runPlayback(signal, name) {
    ...
    // promise (stage/README.md, "Pressing Play moves nothing"); null leaves guardScrollFrame a
    // no-op on the narrow layout, which is allowed to scroll.
    ...
    // 'paused' partway through, the marker reads the same either way — see moss-scene.js's
    // onStageState, which only ever distinguishes 'playing' and 'error' from everything else.
    this.phase = 'done';
    announce(name, this.phase);
  }
}

customElements.define('moss-stage', MossStage);
