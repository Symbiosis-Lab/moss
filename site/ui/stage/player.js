...
// or UI strings — see site/ui/stage/README.md for the module boundaries.
//
 ...
 * may carry it, never what it means beyond that (stage/README.md, "Pace"); the driver alone owns
 * the formula reveals feeds. */
const VERBS = {
  // Types into a named input (stage/README.md, "Scene format") — `{ "typeInto": { "target": "...",
  ...
  // A right-click — how the harvested app's own menus open (stage/README.md, "Scene format").
  context: {
...
// rule below. `reveals` is the only one today (stage/README.md, "Pace": `{ "context": "crumb.file",
// "reveals": 10 }`).
...
/**
 * Plays an `after` chain (stage/README.md, "Scene format"): every entry but the last runs through
 ...
 * loading and lives in moss-stage.js, not here (see the module table). This only runs what it is
 * handed, refusing a chain that repeats a name as a defensive, cheaply-testable last check (a real
 * `after` cycle is rejected earlier, while moss-stage.js is still fetching scene data, so it never
