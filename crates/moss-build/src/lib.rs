...
//!
//! Seeded 2026-08-11 with vault-root identity: root
...
//!
//! Joined 2026-08-18 by the `.moss` **layout** owner: the crate
...
//! above, second type — identity, then layout.
//!
...
//! (scan → render → emit, headless, no tauri) migrates in.
...
//
// The build tree and its private support cluster crossed from the desktop
// app in one enlarged `git mv`.
...
// families below. The desktop app re-exports each at its old path.
pub mod build;
// The editor backend: resolve/tree/content/source-asset cores the
// HTTP command carrier calls; the app keeps the Tauri wrappers + shims.
pub mod editor;
// The QuickJS plugin engine core (open-CLI slice 2): pure rquickjs +
// tokio, app reach injected through `engine::app_host::AppHost`.
pub mod engine;
pub mod i18n;
// Operational modes: the preview server (ops/serve, S1 of the
...
/// Crossed with the build tree; the desktop app keeps its own copy — the lock is
...
/// Wine skip guard: a test may skip under Wine ONLY via
