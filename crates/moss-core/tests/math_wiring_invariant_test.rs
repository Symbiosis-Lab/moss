...
//! for the same pattern. This is moss-core's copy, scoped to this crate's
//! own `src`; moss-build carries the twin for its own `src`.
//!
//! Also carries `shortcode_sub_parses_use_the_callers_config`, which reads
//! only `open/crates/moss-core/src/ast/shortcode_extract.rs`
